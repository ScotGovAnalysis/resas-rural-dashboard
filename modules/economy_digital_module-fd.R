...
load_economic_inactivity_4fold_data <- function(selected_sub_metric = NULL) {
  ...
  tryCatch({
    ...
    processed_data <- raw_data %>%
      ...
      mutate(
        Area_Type= Classification_4fold,
        Classification_4fold = Area,
        # Also create 2-fold for consistency
        Classification_2fold = case_when(
          Area_Type%in% c("Islands & Remote Rural", "Mainly Rural") ~ "Rural",
          Area_Type%in% c("Urban with Substantial Rural areas", "Larger Cities") ~ "Urban",
          TRUE ~ Area
        ),
...
load_registered_businesses_data <- function() {
  ...
  tryCatch({
    ...
    processed_data <- raw_data %>%
      ...
      mutate(
        ...
        Classification_6fold = case_when(
          Area_Type %in% c("Large Urban Areas", "Other Urban Areas", "Accessible Small Towns", 
                           "Remote Small Towns", "Accessible Rural", "Remote Rural") ~ Area,
          TRUE ~ NA_character_
        ),
      ...
      filter(!is.na(Value)) %>%
      select(Year, Area_Type, Classification_2fold, #Classification_3fold, 
  ...
  tryCatch({
    ...
    if (grepl("Table 17", table_name, ignore.case = TRUE)) {
      ...
      rural_urban_data <- data_rows %>%
        ...
        filter(Area_Type%in% c("Islands & Remote Rural", "Mainly Rural", "Urban with Substantial Rural areas", "Larger Cities")) %>%
        mutate(Table = table_name)
    } else {
      # For other tables: Filter for Group = "Rural / Urban Economy Areas"
      rural_urban_data <- data_rows %>%
        filter(Group == "Rural / Urban Economy Areas") %>%
        filter(!is.na(Area)) %>%
        mutate(Table = table_name)
    }
...
process_excel_data_for_dashboard <- function(table_names, selected_sub_metric = NULL) {
  ...
  long_data <- raw_data %>%
    ...
    mutate(
      ...
      #   TRUE ~ Region
      # # ),
      # Classification_2fold = case_when(Region == "Urban Areas" ~ "Urban",
      #                                  Region == "Rural Areas" ~ "Rural"),
...
aggregate_economy_by_classification <- function(processed_data, classification_type = "2-fold", metric_name = NULL) {
  ...
  classification_col <- dplyr::case_when(
    classification_type == "3-fold" && !is.null(metric_name) && metric_name == "Economic inactivity" ~ "Area",
    classification_type == "4-fold" && !is.null(metric_name) && metric_name == "Economic inactivity" ~ "Area",
    classification_type == "2-fold" ~ "Classification_2fold",
    classification_type == "3-fold" ~ "Classification_3fold",
  ...
  # Special handling for 3-fold/4-fold economic inactivity
  if (classification_type == "3-fold" && !is.null(metric_name) && metric_name == "Economic inactivity") {
    return(processed_data %>%
             dplyr::mutate(Classification = classification_type) %>%
             dplyr::select(Year, Area, Value, Data_Source, Sub_Metric))
  }
  
  if (classification_type == "4-fold" && !is.null(metric_name) && metric_name == "Economic inactivity") {
    return(processed_data %>%
             dplyr::mutate(Classification = classification_type) %>%
             dplyr::select(Year, Area, Value, Data_Source))
  }
  
  if (classification_type == "2-fold" && !is.null(metric_name)) {
    return(processed_data %>%
             dplyr::mutate(Classification = classification_type)) 
  }
...
# 
#     regional_data <- processed_data %>%
#       dplyr::filter(!grepl("Scotland|Total", Area_Type, ignore.case = TRUE)) %>%
#       dplyr::group_by(Year, !!rlang::sym(classification_col), Data_Source) %>%
#       dplyr::summarise(Value = sum(Value, na.rm = TRUE), .groups = 'drop') %>%
#       dplyr::rename(Area = !!rlang::sym(classification_col)) %>%
#       dplyr::mutate(Classification = classification_type) %>%
...
#      dplyr::summarise(Value = sum(Value, na.rm = TRUE), .groups = 'drop') %>%
#      dplyr::mutate(Area = "Scotland", Classification = classification_type)
# 
#    aggregated <- dplyr::bind_rows(regional_data, scotland_data)
#  } else {
#  cat("Using average aggregation for", metric_name, "\n")
# 
#   aggregated <- processed_data %>%
...
get_economy_urban_rural_2fold <- function(processed_data, metric_name = NULL) {
 ...
 agg_2fold <- aggregate_economy_by_classification(processed_data, "2-fold", metric_name)
  #agg_2fold <- processed_data |> filter(Classification == "2-fold")
  if(nrow(agg_2fold) == 0) {
    return(list(urban = NA, rural = NA, scotland = NA, year = NA))
  }
  ...
  urban_val <- latest_data %>% filter(Area_Type == "Urban") %>% pull(Value)
  rural_val <- latest_data %>% filter(Area_Type == "Rural") %>% pull(Value)
  scotland_val <- latest_data %>% filter(Area_Type == "Scotland") %>% pull(Value)
    ...
    conditionalPanel(
        ...
        fluidRow(
          box(title = div(style = "display: flex; justify-content: space-between; align-items: center; width: 100%; padding-right: 15px;",
                          ...
                          div(style = "position: absolute; right: 20px;",
                              downloadButton("economy_trend_download", "Download", class = "excel-download-btn"
                              ))),
      ...
      fluidRow(
        box(title = div(style = "display: flex; justify-content: space-between; align-items: center; width: 100%; padding-right: 15px;",
                        ...
                        div(style = "position: absolute; right: 20px;",
                            downloadButton("economy_comparison_download", "Download", class = "excel-download-btn"
                            ))), 
            status = "primary", solidHeader = TRUE, width = 12,
      ...
      fluidRow(
        box(title = div(style = "display: flex; justify-content: space-between; align-items: center; width: 100%; padding-right: 15px;",
                        ...
                        div(style = "position: absolute; right: 20px;",
                            downloadButton("economy_table_download", "Download", class = "excel-download-btn"
                            ))),
...
economy_server <- function(id, values, parent_session = NULL) {
  ...
  output$economy_table_area_filter <- renderUI({
    ...
    if(nrow(economy_values$processed_data) == 0) return(NULL)
  if(input$metric == "Regional Gross Value Added"){agg_data <- economy_values$processed_data}
      else{  agg_data <- aggregate_economy_by_classification(economy_values$processed_data, input$economy_classification_type, input$economy_metric)}
   if(nrow(agg_data) == 0) return(NULL)
    all_areas <- sort(unique(agg_data$Area_Type))
  ...
  output$economy_data_summary <- renderUI({
    ...
    display_name <- get_economy_metric_display_name(input$economy_metric)
    custom_insight <- ifelse(input$economy_metric == "Economic inactivity", 
                             economy_key_insights[[paste(input$economy_metric, input$economy_classification_type) ]],
                             economy_key_insights[[input$economy_metric]])
    custom_notes <- ifelse(input$economy_metric == "Economic inactivity", 
                                                  economy_notes[[paste(input$economy_metric, input$economy_classification_type) ]],
                                                  economy_notes[[input$economy_metric]])
  ...
  output$economy_urban_rate <- renderValueBox({
    ...
    # Skip for Economic inactivity
    if(input$economy_metric == "Economic inactivity") {
      # haven't changed code to include HTML formatting as below - as valuebox not needed. 
  ...
  output$economy_scotland_rate <- renderValueBox({
    ...
    if(input$economy_metric == "Economic inactivity") {
      # haven't changed code to include HTML formatting as below - as valuebox not needed. 
      return(valueBox(value = "N/A", subtitle = "Scotland", icon = icon("flag"), color = "aqua"))
    }
   ...
   # valueBox(value = val, subtitle = paste(scotland_summary$label, year), icon = icon("flag"), color = "aqua")
    valueBox(
      value = tagList(
        tags$div(style = "font-size:20pt; font-weight:normal; min-height:40px;", paste(scotland_summary$label, year)),
        tags$div(style = "font-size:22pt; font-weight:bold;", val)
      ),
      subtitle = NULL,
      icon = icon("flag"),
  ...
  output$economy_urban_rural_gap <- renderValueBox({
    ...
    if(input$economy_metric == "Economic inactivity") {
      return(
        valueBox(value = "N/A", subtitle = "Urban-Rural Gap", icon = icon("balance-scale"), color = "maroon") 
        # haven't changed code to include HTML formatting as below - as valuebox not needed. 
        )
    }
    ...
    #valueBox(value = val, subtitle = "Urban-Rural Gap", icon = icon("balance-scale"), color = "maroon")
    valueBox(
      value = tagList(
  ...
  output$economy_trend_chart <- renderPlotly({
    ...
    tryCatch({
      ...
      if (is_percentage) {
        ...
        y_label <- if(input$economy_metric == "Regional Gross Value Added"){"Millions of pounds" }
      ...
      # Create plot with 45-degree rotated labels
  ...
  # Economy trend download
  output$economy_trend_download <- downloadHandler(
    filename = function() {
      metric <- gsub("[^A-Za-z0-9]", "_", input$economy_metric)
      paste0("Economy_Trend_", metric, "_", input$economy_classification_type, "_", Sys.Date(), ".csv")
    },
    content = function(file) {
      req(economy_values$processed_data, input$economy_classification_type)
      
      data <- aggregate_economy_by_classification(
        economy_values$processed_data,
        input$economy_classification_type,
        input$economy_metric
      ) %>%
  ...
  # Economy Comparison download
  
  output$economy_comparison_download <- downloadHandler(
    filename = function() {
      metric <- gsub("[^A-Za-z0-9]", "_", input$economy_metric)
      year <- if (input$economy_metric == "Economic inactivity" && input$economy_classification_type == "3-fold") {
        "2023"
      } else {
        input$economy_selected_year
      }
      paste0("Economy_Comparison_", metric, "_", input$economy_classification_type, "_", year, "_", Sys.Date(), ".csv")
    },
    content = function(file) {
      req(economy_values$processed_data, input$economy_classification_type)
      
      data <- if (input$economy_metric == "Economic inactivity" && input$economy_classification_type == "3-fold") {
        economy_values$processed_data %>%
          select(Year, Area, Value, Data_Source) %>%
          arrange(Area)
      } else {
        req(input$economy_selected_year)
        aggregate_economy_by_classification(
          economy_values$processed_data,
          input$economy_classification_type,
          input$economy_metric
        ) %>%
  ...
  # Economy table download
  output$economy_table_download <- downloadHandler(
    filename = function() {
      metric <- gsub("[^A-Za-z0-9]", "_", input$economy_metric)
      paste0("Economy_Table_", metric, "_", input$economy_classification_type, "_", Sys.Date(), ".csv")
    },
    content = function(file) {
      req(economy_values$processed_data, input$economy_classification_type)
      ...
      if (!is.null(input$economy_table_year_filter) && input$economy_table_year_filter != "all") {
        data <- data %>% filter(Year == as.numeric(input$economy_table_year_filter))
      }
      if (!is.null(input$economy_table_area_filter) && input$economy_table_area_filter != "all") {
        data <- data %>% filter(Area_Type== input$economy_table_area_filter)
      }
  ...
  output$economy_data_table <- DT::renderDataTable({
    ...
    if(input$economy_metric == "Economic inactivity") {
      if(input$economy_classification_type == "3-fold") {
        ...
        if(!is.null(input$economy_table_area_filter) && input$economy_table_area_filter != "all") {
          filtered_data <- filtered_data %>% filter(Area_Type== input$economy_table_area_filter)
        }
