...
agriculture_metrics <- list(
  ...
  "Area of woodland creation and new woodland creation" = list(
  ...
  "Diversified Activity And Incomes" = list(
    ...
    classifications = c("Scotland"),
    full_name = "Diversified activity and incomes of farms (5 year matched sample)",
    unit = "Various",
    has_sub_metrics = TRUE,
    no_bar_chart = TRUE
  ),
...
agriculture_key_insights <- list(
  ...
  "GVA (Agri, Forestry, Fishing)" = "Regional Gross Value added by agriculture, forestry and fishing sectors has seen the highest growth over the recent years in mainly rural areas (£1,511m in 2018 to £2310m in 2022). Other regions remain relatively stable with urban with substantial rural areas contributing the second largest regional GVA (£532m) and larger cities the smallest (£111m) in 2022.",
  ...
  "Area of woodland creation and new woodland creation" = "Scotland consistently has the highest area of woodland creation in the UK (15.1 thousand hectares in 2023-24 compared to 4.1 thousand hectares in England). Between 2022-23 and 2023-24 Scotland saw a large increase in woodland creation, going from 8.2 to 15.1 thousand hectares.",
...
agriculture_notes <- list(
  ...
  "Fish Stocks" = "",
  "Greenhouse Gas Emissions" = "1. Including international aviation and shipping. 2. Values in MtCO2e. 3. LULUCF stands for land use, land-use change, and forestry.",
  "Area of woodland creation and new woodland creation" = "",
  ...
  "New To Crofting" = "The count for 1 April 2024 to 31 March 2025 is provisional",
  "Resident Crofters" = "",
  "Agritourism Value" = "1. Value for 2021/22 is a business estimate. 2. VisitScotland will undertake a new agritourism tracker in 2025 that is to be published in 2026. 3. Agritourism is defined in Scotland as 'tourism or leisure on a working farm, croft, or estate, which produces food'. "
)
...
load_new_crofting_data <- function() {
  ...
  tryCatch({
    ...
    periods <- as.character(raw_data[2, 2:7])
    periods <- periods[!is.na(periods) & periods != ""]
    
    values <- as.numeric(raw_data[3, 2:7])
    ...
    period_order <- c("Oct 2021- Sep 2022", "April 2022 - Mar 2023", "Oct 2022- Sep 2023", "Apr 2023- Mar 2024",
                      "Oct 2023- Sep 2024", "Apr 2024- Mar 2025*" )
    ...
    year_mapping <- c(
      ...
      "Apr 2023- Mar 2024" = 2024,
      "Oct 2023- Sep 2024" = 2025,
      "Apr 2024- Mar 2025*" = 2026
    )
          ...
          fluidRow(
            box(
              ...
              div(
                ...
                div(
                  class = "agriculture-metrics-card",
                  onclick = "Shiny.setInputValue('agriculture_metric_select', 'Area of woodland creation and new woodland creation ', {priority: 'event'});",
                  div(
                    style = "display: flex; align-items: flex-start;",
...
agriculture_server <- function(id, values, parent_session = NULL) {
  ...
  observe({
    ...
    } else if (input$agriculture_metric == "Area of woodland creation and new woodland creation") {
  ...
  output$agriculture_data_summary <- renderUI({
    ...
    source_info <- switch(input$agriculture_metric,
                          ...
                          "Area of woodland creation and new woodland creation" = list(
                            text = "Forestry Statistics 2024, Forest Research",
    ...
    if (input$agriculture_metric %in% c("Farm Income", "Diversified Activity And Incomes")) {
      ...
      if (input$agriculture_classification_type %in% c("UK", "Scotland")) {
        ...
        if (input$agriculture_classification_type == "UK") {
          ...
          if (input$agriculture_metric == "Area of woodland creation and new woodland creation") {
            p <- p +
              scale_x_continuous(
                breaks = function(x) {
                  if (length(x) == 0 || all(is.na(x))) return(c())
          ...
          if (input$agriculture_metric == "New To Crofting") {
            p <- p +
              ...
              labs(x = "Period", y = get_agriculture_y_label(input$agriculture_metric)) 
  ...
  output$agriculture_comparison_chart <- renderPlotly({
    ...
    tooltip_text <- case_when(
      ...
      input$agriculture_metric == "Area of woodland creation and new woodland creation" ~ paste0("Country: ", selected_data$Area, "<br>Area: ", selected_data$Value_Rounded, " thousand hectares"),
