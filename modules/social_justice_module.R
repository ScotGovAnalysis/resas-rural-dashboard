...
social_justice_metrics <- list(
  ...
  "Level of fuel poverty" = list(
    file_6fold = "social_justice/fuel_poverty.xlsx",
    classifications = c("6-fold"),
    full_name = "Proportion (%) of homes in fuel poverty"
  ),
...
social_justice_key_insights <- list(
  "Relative poverty 2-fold" =  "The proportion (%) of people who are in relative poverty in rural areas has risen 4 percentage points in the past ten years between 2014 and 2024, from 13% to 17%. It remains lower than the proportion (%) of people in relative poverty in urban areas which rose 2 percentage points from 19% in 2014 to 21% in 2024.",
  "Relative poverty 6-fold" = "In 2021-2024, the proportion (%) of people who were in relative poverty was highest in Other Urban Areas (22%) and lowest in Remote Rural Areas (13%)",
  "Relative child poverty" = "The proportion (%) of children who are in relative poverty in rural areas has risen 5 percentage points in the past ten years between 2014 and 2024, from 16% to 21%. It remains lower than the proportion (%) of children in relative poverty in urban areas which rose 2 percentage points from 22% in 2014 to 24% in 2024." ,
  "Level of fuel poverty" = "In 2023, the proportion of homes in fuel poverty was highest in remote rural areas (44%) and lowest in accessible rural areas (31%). Between 2022 and 2023 the proportion of homes in fuel poverty rose across Scotland and also in most of the urban-rural classifications except for accessible small towns (where it fell from 34% to 32%) and remote rural areas (from 47% to 44%).", 
  ...
  "I can influence decisions affecting my local area" = "In 2023, the percentage of people who felt they could influence decisions affecting their local area was highest in remote rural areas (20%) followed by remote small towns (19%) and lowest in other urban areas (18%)." ,
  "I would like to be more involved in decisions" = "In 2023, the percentage of people who would like to be more involved in the decisions their council makes that affect their local area was highest in large urban reas (35%) followed by accessible small towns (28%) and lowest in remote rural areas (23%).",  
  "Managing financially" = "In 2023, the percentage of households managing well financially (answering, very well or quite well) was highest in remote rural areas (63%) and lowest in large urban areas (49%).",
  "Community ownership" =   "In 2023, the number of assets in community ownership was highest in remote rural areas (513) and lowest in remote small towns (25)."  
)
...
social_justice_notes <- list(
  ...
  "Level of fuel poverty" = "1. 2021 to 2022 urban rural categories are based on the 2020 Scottish Government Urban/Rural classification. 2. 2016 to 2019 figures use 2013/2014 urban rural classification, 2011 definition of data zones. 3. Dual figures exist for 2017 fuel poverty, most recent figures from 2018 source have been used. 
  4. The time period covered by these statistics  means that time series results was affected by the coronavirus (COVID-19) pandemic. 5. 6. Separate island fuel poverty data unavailable but will be by 2025/26", 
  "Housing affordability" = "Three year averages are used except for 2018-21, 2019-22 and 2020-23 where only two years of data are included due to the impact of the pandemic on the survey in 2020-21.",
  "Influence over local decisions" = "1. Due to pandemic-related changes to the Scottish Household Survey, figures for 2020 and 2021 are not comparable with other years. For interest, data for these years has been included below. 2. Scottish Household Survey uses most relevant SG Urban Rural Classification for each year", 
  "Managing financially" = "1. Results from 2020 and 2021 are not considered comparable to other years and have been excluded", 
  "Community ownership" =   ""  
)
...
load_poverty_data <- function(filename, data_type) {
 ...
 raw_data <- read.csv(filepath)
    ...
    colnames(raw_data) <- gsub("^X", "", gsub("\\.", "-", colnames(raw_data)))
    ...
    pivot_longer(-Group,
     ...
     mutate(
         Value = case_when(
           ...
           str_detect(Value, "[u]") ~ NA_character_,
          .default = c(gsub("%", "", Value)) # "12%", "1,234%", "  12 % " → 12
         )) |> 
      mutate(
        ...
        Classification = case_when(Area_Type %in% c("Scotland", "All", "Urban", "Rural") ~ "2-fold",
                                   Area_Type %in% c("Scotland", "All", "Large Urban Areas",
                                                    "Other Urban Areas", "Accessible Small Towns",
                                                    "Remote Small Towns",
                                                    "Accessible Rural Areas",
                                                    "Remote Rural Areas") ~ "6-fold",
                                   .default = NA_character_),
     Data_Source = "Scottish Government Relative Poverty"
     ) %>%
     # filter(!is.na(Value), !is.na(Year)) %>%
      select(Year,
             Classification,
             Year_Range, Area = Area_Type, 
             Value,
             Data_Source)
      #        ) |> 
      # filter(Classification == classification_type )
      # 
    cat(paste("Loaded", nrow(processed_data), "poverty records from", "\n"))
    return(processed_data)
    
  }, error = function(e) {
    cat("Error loading poverty data from", ":", e$message, "\n")
    return(data.frame())
  })
}

# Load fuel poverty data 
load_fuel_poverty_data <- function(filename) {
  filepath <- "social_justice/fuel_poverty.xlsx"
  cat("Loading fuel poverty from:", filepath, "\n")
  ...
  tryCatch({
    ...
    raw_data <-  read_excel(filepath, sheet = "Indicator 9.5", col_names = F)
    processed_data <- raw_data 
    
    # 1) Capture first row as names
    new_names <- processed_data %>% slice(1) %>% unlist() %>% as.character()
    
    # 2) Set names and drop the first row
   processed_data <- processed_data %>%
     ...
     pivot_longer(cols = -1,
      ...
      mutate(
        Area = case_when(
          ...
          Region == "All Scotland" ~ "Scotland"
        ),
        Year = as.numeric(Year),
        Value = as.numeric(Value) * 100,
        Data_Source = "Scottish House Condition Survey"
      ) %>%
    ...
    cat(paste("Loaded", nrow(processed_data), "fuel poverty\n"))
    ...
    cat("Error loading fuel poverty data:", e$message, "\n")
    return(data.frame())
  })
}
...
load_housing_affordability_data <- function() {
  ...
  tryCatch({
    ...
    processed_data <- data_rows %>%
      ...
      select(Year, Area, Value, Data_Source, Year_Range, Period)
...
load_perceptions_data_new <- function(selected_statement = NULL) {
  ...
  tryCatch({
    ...
    # Process the data
    processed_data <- raw_data %>% select(-Council) |> 
      filter(!is.na(Year), !is.na(Statement), Statement != "") %>%
...
return(load_poverty_data("poverty_adult_6_fold.csv", "adult"))

  } else if (metric_name == "Relative child poverty") {
    return(load_poverty_data("poverty_child_6_fold.csv", "child"))

  }  else
    
    if (metric_name == "Level of fuel poverty") {
    return(load_fuel_poverty_data("fuel_poverty.xlsx"))
...
# library(dplyr)
# 
# simple_aggregate_social_justice_data <- function(processed_data,
#                                                  classification_type = "2-fold",
#                                                  metric = NULL) {
#   if (nrow(processed_data) == 0) {
#     return(data.frame())
#   }
#   
#   # Metrics that may use 2-fold or 6_fold based on user selection
#   poverty_metrics <- c("Relative poverty", "Relative child poverty")
# 
#   # Determine classification to apply:
#   # - Poverty metrics: use user-selected classification_type ("2-fold" or "6_fold")
#   # - Non-poverty metrics: do not filter on Classification (column doesn't exist)
# #  is_poverty <-  input$social_justice_metric %in% poverty_metrics
#   #effective_classification <- if (is_poverty) {classification_type} else {"2-fold"}
# 
#   # Conditionally filter ONLY if:
#   #  - metric is poverty, and
#   #  - the data has a 'Classification' column
#   filtered_data <- if (input$social_justice_metric %in% poverty_metrics && ("Classification" %in% names(processed_data))) {
#     processed_data %>%
#       filter(Classification == input$social_justice_classification_type)
#   } else {
#     processed_data
#   }
#   
#   # Aggregate (adjust the grouping fields if needed)
#   filtered_data %>%
#     group_by(Year, Area, Data_Source) %>%
#     summarise(Value = mean(Value, na.rm = TRUE), .groups = "drop")
# }
          ...
          fluidRow(
            box(
              ...
              div(
                ...
                # div(
                #   class = "social-justice-metrics-card",
                #   onclick = "Shiny.setInputValue('social_justice_metric_select', 'Transport affordability', {priority: 'event'});",
                #   div(
                #     style = "display: flex; align-items: flex-start;",
                ...
                div(
                  class = "social-justice-metrics-card",
                  onclick = "Shiny.setInputValue('social_justice_metric_select', 'Level of fuel poverty', {priority: 'event'});",
                  div(
                    style = "display: flex; align-items: flex-start;",
                    icon("child", class = "social-justice-metric-icon"),
                    div(
                      div(class = "social-justice-metric-title", "Level of fuel poverty"),
                      div(class = "social-justice-metric-description",
...
social_justice_server <- function(id, values, parent_session = NULL) {
  ...
  #   else if (input$social_justice_metric %in% c("Relative poverty", "Relative child poverty"))
  # 
  #    {social_justice_values$processed_data <- load_social_justice_data_simple(
  #       input$social_justice_metric,
  #       input$social_justice_classification_type
  ...
  output$social_justice_data_summary <- renderUI({
    ...
    # Only show custom insights for metrics 
   if(input$social_justice_metric == "Influence over local decisions"){custom_insight <- social_justice_key_insights[[input$selected_influence_statement]]}
    else if(input$social_justice_metric %in% c("Relative poverty"))
      {custom_insight <- social_justice_key_insights[[paste(input$social_justice_metric, input$social_justice_classification_type) ]]}
    else{custom_insight <- social_justice_key_insights[[input$social_justice_metric]]}
    ...
    source_info <- switch(input$social_justice_metric,
                          "Relative poverty" = list(
                            text = "Family Resources Survey, Poverty and Income Equality in Scotland 2021-24, Scottish Government",
                            url = "https://data.gov.scot/poverty/#All_people/"
                          ),
                          "Relative child poverty" = list(
                            text = "Family Resources Survey, Poverty and Income Equality in Scotland 2021-24, Scottish Government", 
                            url = "https://view.officeapps.live.com/op/view.aspx?src=https%3A%2F%2Fdata.gov.scot%2Fpoverty%2Fxls%2Fdata2025.xlsx&wdOrigin=BROWSELINK"
                          ),
                          "Level of fuel poverty" = list(
                          ...
                          "Housing affordability" = list(
                            text = "Scottish Government analysis of data from the Family Resources Survey",
                            url = "https://view.officeapps.live.com/op/view.aspx?src=https%3A%2F%2Fwww.gov.scot%2Fbinaries%2Fcontent%2Fdocuments%2Fgovscot%2Fpublications%2Fstatistics%2F2025%2F05%2Fadditional-poverty-analysis-2025%2Fdocuments%2Fhousing-affordability-ratio-by-urban-rural%2Fhousing-affordability-ratio-by-urban-rural%2Fgovscot%253Adocument%2FHousing%252Baffordability%252Burban%252Brural.xlsx&wdOrigin=BROWSELINK"
                          ),
                          "Influence over local decisions" = list(
  ...
  output$social_justice_trend_chart <- renderPlotly({
    ...
    tryCatch({
      ...
      if(input$social_justice_metric %in% poverty_metrics) {
       agg_data <-  simple_aggregate_social_justice_data(social_justice_values$processed_data, input$social_justice_classification_type) 
       #agg_data <- agg_data |> filter(Classification == input$social_justice_classification_type )
       }
      else{
      agg_data <- simple_aggregate_social_justice_data(social_justice_values$processed_data, input$social_justice_classification_type)}
      ...
      is_percentage_data <- input$social_justice_metric %in% c("Relative poverty", "Relative child poverty", "Level of fuel poverty", "Housing affordability", "Influence over local decisions", "Managing financially")
      ...
      # Create plot
    if(input$social_justice_metric %in% c("Level of fuel poverty", "Influence over local decisions",
                                          "Managing financially"))
    ...
    p <- ggplot(agg_data1, aes(x = Year, y = Value, color = Area, group = Area, text = tooltip)) +
      ...
      annotate(
        ...
        label = "Data for 2020 and 2021 were affected by the coronavirus (COVID-19) pandemic.",  # stacked text using newline
      ...
      p <- ggplot(agg_data, aes(x = Year, y = Value, color = Area, group = Area, text = tooltip)) +
        ...
        if(input$social_justice_metric %in% c("Relative poverty", "Relative child poverty", "Housing affordability")){scale_x_continuous(
        ...
        else{scale_x_continuous(breaks = function(x) {
          if(length(x) == 0 || all(is.na(x))) return(c())
          seq(floor(min(x, na.rm = TRUE)), ceiling(max(x, na.rm = TRUE)), by = 1)
        })}
}
