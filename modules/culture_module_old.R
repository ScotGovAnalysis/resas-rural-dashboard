...
culture_key_insights <- list(
  "Attendance at cultural events and visiting places of culture" = "Attendance at cultural events and visiting places of culture (any activity including cinema) is at similar levels between rural and urban communities, with slightly higher proportions in urban (76.9%) as compared to rural (76.2%). There has been a drop in participation in cultural activities from 2019 to 2022 related to the COVID pandemic. However, participation in cultural activities has been recovering with higher rates across all 6 categories of urban/rural classification from 2022 to 2023, except for remote small towns which saw a large drop (79.1% in 2022 to 66.3% in 2023).",
  
  "Participation in any cultural activity" = "Participation in any cultural activity including reading in 2023 was the highest in accessible rural areas (79.6%) and lowest in other urban areas (71%). Rural areas show generally higher levels of participation in any cultural activity including reading than urban areas (78.8% vs 73%).",
  
  "Gaelic speakers" = "Population speaking Gaelic has been growing between 2011 and 2022 across all classifications except for Islands and Remote Rural Areas which saw a continued drop in Gaelic speakers. The increase in Gaelic speakers in urban areas is of particular interest, with both urban with substantial rural areas and larger cities seeing a growth of around 1.5x each.",
  
  "Visits to attractions" = "Large urban areas see the highest number of visits to attractions (24.6 million) recovering to pre-COVID levels in 2023 and 2024. Other areas experienced a smaller drop due to the pandemic but appear to have recovered as well."
)

# Static Notes for each culture metric
culture_notes <- list(
  "Attendance at cultural events and visiting places of culture" = "The data for 2020 and 2021 is missing because of the COVID pandemic.",
  
  "Participation in any cultural activity" = "The data for 2020 and 2021 is missing because of the COVID pandemic.",
  
  "Gaelic speakers" = 'The data shows raw numbers of Gaelic speakers by region. 
  "Speaks Gaelic" category includes "Speaks, reads and writes Gaelic", "Speaks but does not read or write Gaelic" and "Speaks and reads but does not write Gaelic". "Other combination of skills in Gaelic" is excluded.',
                  ...
                  div(
                    ...
                    div(
                      ...
                      div(class = "culture-metric-description", 
                          "Participation rates across 18 different cultural activities including cinema, theatre, museums, live music, and festivals. Shows how urban and rural communities engage with Scotland's cultural offerings.")
                    )
                  )
                ),
    ...
    conditionalPanel(
      ...
      fluidRow(
        box(title = div(style = "display: flex; justify-content: space-between; align-items: center; width: 100%; padding-right: 15px;",
                        uiOutput("culture_trend_title"),
                        div(style = "position: absolute; right: 20px;",
                            tags$a(class = "excel-download-btn", href = "#", 
                                   onclick = "Shiny.setInputValue('culture_trend_download', Math.random());",
      ...
      fluidRow(
        box(title = div(style = "display: flex; justify-content: space-between; align-items: center; width: 100%; padding-right: 15px;",
                        ...
                        div(style = "position: absolute; right: 20px;",
                            tags$a(class = "excel-download-btn", href = "#", 
                                   onclick = "Shiny.setInputValue('culture_comparison_download', Math.random());",
                                   icon("download"), "Download"))), 
            status = "primary", solidHeader = TRUE, width = 12,
      ...
      fluidRow(
        box(title = div(style = "display: flex; justify-content: space-between; align-items: center; width: 100%; padding-right: 15px;",
                        ...
                        div(style = "position: absolute; right: 20px;",
                            tags$a(class = "excel-download-btn", href = "#", 
                                   onclick = "Shiny.setInputValue('culture_table_download', Math.random());",
                                   icon("download"), "Download"))), 
...
culture_server <- function(id, values, parent_session = NULL) {
  ...
  output$culture_data_summary <- renderUI({
    ...
    if (!is.null(custom_insight)) {
      insight_content <- div(
        h4("Key Finding", style = "margin: 0 0 10px 0; color: #155724; font-weight: 600;"),
        p(custom_insight, style = "font-size: 1.05em; line-height: 1.6; margin: 0 0 15px 0; color: #155724;")
      )
      ...
      if (!is.null(custom_notes) && custom_notes != "" && custom_notes != "notes placeholder") {
        insight_content <- tagList(
          ...
          div(
            strong("Notes:", style = "font-size: 0.9em; color: #155724;"),
            p(custom_notes, style = "font-size: 0.9em; line-height: 1.4; margin: 5px 0 15px 0; color: #155724;")
          )
        )
      }
      ...
      insight_content <- tagList(
        ...
        hr(style = "border-top: 1px solid #c3e6cb; margin: 15px 0 10px 0;"),
        div(
          style = "font-size: 0.9em; color: #155724;",
    ...
    return(div(h4("Culture Data Loaded"), p(paste("Showing data for:", display_name))))
  })
  ...
  # Download handlers
  observeEvent(input$culture_trend_download, {
    req(culture_values$processed_data, input$culture_classification_type)
    ...
    if (nrow(agg_data) > 0) {
      ...
      filename <- paste0("Culture_Trend_", metric_name, "_", input$culture_classification_type, "_", Sys.Date(), ".csv")
  ...
  observeEvent(input$culture_comparison_download, {
    req(culture_values$processed_data, input$culture_classification_type, input$culture_selected_year)
    ...
    if (nrow(selected_data) > 0) {
      ...
      metric_name <- gsub("[^A-Za-z0-9]", "_", input$culture_metric)
      filename <- paste0("Culture_Comparison_", metric_name, "_", input$culture_classification_type, "_", input$culture_selected_year, "_", Sys.Date(), ".csv")
  ...
  observeEvent(input$culture_table_download, {
    req(culture_values$processed_data, input$culture_classification_type)
    ...
    if (nrow(agg_data) > 0) {
      ...
      metric_name <- gsub("[^A-Za-z0-9]", "_", input$culture_metric)
      filename <- paste0("Culture_Table_", metric_name, "_", input$culture_classification_type, "_", Sys.Date(), ".csv")
